...
13. Click on the created feature (street or building) in the sketch map. Click on the corresponding feature in the base map. Select the Align button in the center to align the features in sketch map to its corresponding feature in the metric map. **Alignment is possible only in the edit mode**
...
### Results
 The Analysis process takes a longer time. Please **wait** till a modal appears and the completeness, generalization and qualitative accuracy values of all sketch maps are computed and filled in. Once the values of all the sketch maps are available, the detailed results can be downloaded using the "Download" button at the end of the results (**Download only after values for all sketch maps are displayed**).
### Save Project
The project could be saved anytime using the "Download Project" available in the "File" menu at the top-left. It is suggested to **save the project time to time** so that the work is not lost due to any network issue or system restart.
The downloaded project could be uploaded later to continue the analysis process.
### Upload Project
